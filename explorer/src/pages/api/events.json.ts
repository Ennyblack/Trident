...
export const GET: APIRoute = async ({ url }) => {
  ...
  try {
    ...
    else if (status === 401 || status === 403 || code === 'UNAUTHORIZED') reason = 'unauthorized';
    ...
    const messages: Record<UnreachableReason, string> = {
      ...
      unauthorized: "The explorer's server key is not configured. This is on us, not you.",
      network: 'Could not reach the Trident indexer. Please check your connection and retry.',
    ...
    } satisfies ExplorerEventsResponse, httpStatus);
  }
};